...
# ── Farbpalette (angelehnt an die Vertriebsplattform) ────────────────────────
BG       = "#0a0a0c"
SIDEBAR  = "#0c0c0f"
CARD     = "#141417"
CARD_HI  = "#1b1b20"
...
# Status-Tints: (Hintergrund, Rand, Vordergrund)
STATUS = {
...
class LeadMachine:
    ...
    def _build_sidebar(self):
        ...
        mark = ctk.CTkFrame(logo, width=34, height=34, corner_radius=9,
                            fg_color=ACCENT)
        mark.pack(side="left")
        mark.pack_propagate(False)
        ctk.CTkLabel(mark, text="L", font=self.f_logo,
                     text_color="#ffffff").pack(expand=True)
        ctk.CTkLabel(logo, text="Leadmachine", font=self.f_logo,
        ...
        self._nav_item(sb, "import", "CSV importieren", self._import_csv,
                       toggle=False)

        # Bottom: Fortschrittskarte
        ...
        ctk.CTkLabel(inner, text="ABGEARBEITET", font=self.f_label,
                     text_color=MUTED).pack(anchor="w")
        self.sb_prog_lbl = ctk.CTkLabel(inner, text="0 / 0", font=self.f_section,
                                        text_color=TEXT)
    ...
    def _nav_item(self, parent, key, label, command, toggle=True):
        ...
        btn = ctk.CTkButton(row, text="   " + label, anchor="w", font=self.f_nav,
                            ...
                            text_color=SUBTLE, corner_radius=8, height=38,
                            command=command)
    ...
    def _build_main(self):
        ...
        ctk.CTkLabel(tl, text=date_str, font=self.f_sub,
                     text_color=MUTED, anchor="w").pack(anchor="w", pady=(2, 0))
        ...
        self.content.grid(row=1, column=0, sticky="nsew", padx=28, pady=(0, 24))
    ...
    def _kpi_card(self, parent, col, key, label, tint, fg, glyph):
        ...
        icon = ctk.CTkFrame(pad, width=42, height=42, corner_radius=10,
                            fg_color=tint)
        icon.pack(side="left")
        icon.pack_propagate(False)
        ctk.CTkLabel(icon, text=glyph, font=ctk.CTkFont(size=18),
                     text_color=fg).pack(expand=True)

        txt = ctk.CTkFrame(pad, fg_color="transparent")
        txt.pack(side="left", padx=12, fill="x", expand=True)
        ctk.CTkLabel(txt, text=label.upper(), font=self.f_label,
                     text_color=MUTED, anchor="w").pack(anchor="w")
        var = tk.StringVar(value="0")
        self.kpi_vars[key] = var
        ctk.CTkLabel(txt, textvariable=var, font=self.f_kpi,
                     text_color=TEXT, anchor="w").pack(anchor="w", pady=(1, 0))

    def _section(self, parent, title, link=None):
        card = ctk.CTkFrame(parent, fg_color=CARD, corner_radius=16,
                            border_width=1, border_color=BORDER)
        head = ctk.CTkFrame(card, fg_color="transparent")
        head.pack(fill="x", padx=22, pady=(18, 0))
        ctk.CTkLabel(head, text=title, font=self.f_section,
    ...
    def _build_lead_panel(self, panel):
        ...
        # Kopf: Fortschritt + Status-Pille
        head = ctk.CTkFrame(body, fg_color="transparent")
        head.pack(fill="x")
        self.lead_pos = ctk.CTkLabel(head, text="", font=self.f_label,
                                     text_color=MUTED)
        ...
        ctk.CTkButton(sec, text="Google Maps", command=self._open_maps,
                      font=self.f_btn, fg_color=CARD_HI, hover_color=BORDER,
                      text_color=SUBTLE, corner_radius=9, height=36,
                      border_width=1, border_color=BORDER, width=130).pack(side="left")
        ctk.CTkButton(sec, text="Überspringen", command=self._skip,
                      font=self.f_btn, fg_color=CARD_HI, hover_color=BORDER,
                      text_color=MUTED, corner_radius=9, height=36,
                      border_width=1, border_color=BORDER, width=130).pack(
                          side="left", padx=(8, 0))

        # Notizen
        ctk.CTkLabel(body, text="NOTIZEN", font=self.f_label, text_color=MUTED,
                     anchor="w").pack(fill="x", pady=(16, 5))
        self.notes_text = ctk.CTkTextbox(body, height=58, font=self.f_body,
        ...
        for i, st in enumerate(order):
            ...
            ctk.CTkButton(actions, text=cfg["label"], font=f_action,
                          ...
                          text_color=cfg["fg"], border_width=1,
                          border_color=cfg["bd"], corner_radius=10,
                          height=46).grid(row=0, column=i, sticky="nsew",
    ...
    def _detail_row(self, parent, r, label, var, color, cmd):
        ...
        ctk.CTkLabel(parent, text=label, font=self.f_label, text_color=MUTED,
                     anchor="w", width=90).grid(row=r, column=0, sticky="w",
                                                pady=5)
    ...
    def _build_pipeline(self, panel):
        ...
        for st in ["pending", "nicht_erreicht", "interessiert", "rueckruf",
            ...
            hdr.pack(fill="x")
            dot = ctk.CTkFrame(hdr, width=9, height=9, corner_radius=5,
                               fg_color=cfg["fg"])
            ...
            ctk.CTkLabel(hdr, textvariable=cnt, font=self.f_bodyb,
    ...
    def _build_list_view(self):
        ...
        ctk.CTkLabel(bar, text="STATUS", font=self.f_label,
        ...
        ctk.CTkLabel(bar, text="SUCHE", font=self.f_label,
                     text_color=MUTED).pack(side="left", padx=(20, 8))
        ...
        self.list_count = ctk.CTkLabel(bar, text="", font=self.f_small,
                                       text_color=MUTED)
        ...
        cols = ("title", "phone", "city", "category", "rating", "status")
        labels = ("FIRMA", "TELEFON", "STADT", "BRANCHE", "BEWERTUNG", "STATUS")
        widths = (300, 165, 135, 195, 115, 150)
        self.tree = ttk.Treeview(wrap, columns=cols, show="headings",
                                 style="LM.Treeview", yscrollcommand=sb.set)
    ...
    def _update_stats(self):
        ...
        for l in self.leads:
            counts[l.get("status", "pending")] = counts.get(
                l.get("status", "pending"), 0) + 1
        ...
        for st, (cnt, bar) in self.pipe.items():
            ...
            bar.set((counts[st] / mx) if mx else 0)
    ...
    def _show_lead(self, lead):
        ...
        self.lead_pos.configure(text=f"LEAD {idx} VON {len(q)}")
    ...
    def _import_csv(self):
        ...
        if not paths:
            return
        existing = {l["phone_normalized"] for l in self.leads
                    if l.get("phone_normalized")}
        ...
        if duplicates:
            msg += f"\n{duplicates} Duplikate übersprungen (gleiche Telefonnummer)"
        if skipped:
        ...
        for lead in self.leads:
            ...
            self.tree.insert("", "end", iid=lead["id"], tags=(lead["status"],),
                             values=(lead.get("title", ""), lead.get("phone", ""),
                                     ...
                                     stars, cfg.get("label", lead["status"])))
            shown += 1
